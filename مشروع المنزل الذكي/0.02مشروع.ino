...
// ******* 1. تعريف أطراف الدوائر (تطابق مع جدول توصيلاتك) *******

// البلوتوث HC-05 
const int BT_RX_PIN = 2; // مستقبل الأردوينو -> يتصل بـ TXD في البلوتوث
const int BT_TX_PIN = 3; // مرسل الأردوينو -> يتصل بـ RXD في البلوتوث

// مستشعر المسافة HC-SR04 (Trigger/Echo)
const int TRIGGER_PIN = 10; // موصل لديك بـ Pin 10
const int ECHO_PIN = 11;    // موصل لديك بـ Pin 11

// محرك السيرفو 
const int SERVO_PIN = 9; // موصل لديك بـ Pin 9
...
const int BUZZER_PIN = 8; // موصل لديك بـ Pin 8

// ******* 2. تعريف الثوابت وقيم الزوايا *******
const int MAX_DISTANCE = 200; // 2 متر
const int OPEN_ANGLE = 170;   // زاوية الفتح
const int CLOSE_ANGLE = 10;   // زاوية الإغلاق
const int SAFETY_THRESHOLD = 20; // حد الأمان (20 سم)
...
// ******* 4. المتغيرات الحالة *******
bool isWindowOpen = false; // لمتابعة حالة النافذة
...
void setup() {
  ...
  // إغلاق النافذة في البداية (لتثبيت السيرفو والـ LED)
